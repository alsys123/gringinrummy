...
function cpuTurn() {
    ...
    if (cDW === 0) return cpuGin();
    if (cDW <= 7) return cpuKnock();
    ...
    if (!drawn) {
        if (!game.stock.length) return stockDepletionResolution();
        drawn = game.stock.pop();
        log("CPU drew from stock.");
    }

    const sourceElem = drawnFromDiscard ? el.discard : el.stock;

    // ⭐ STEP 1 — Animate draw
    animateCpuDraw(sourceElem, drawn, () => {

        // ⭐ STEP 2 — Apply draw AFTER animation
        game.cpu.push(drawn);

        // ⭐ STEP 3 — Choose discard
        const idx = cpuChooseDiscardIndex();
        const card = game.cpu[idx];

        // ⭐ STEP 4 — Animate discard
        animateCpuDiscard(idx, card, () => {

            // ⭐ STEP 5 — Apply discard AFTER animation
...
const sourceElem = drawnFromDiscard
  ? el.discard
  : el.stock;
...
function animateCpuDraw(sourceElem, card, onDone) {
  const rect = sourceElem.getBoundingClientRect();

  // Create flying card
  const flying = cardFace(card); // or a back card if CPU is hidden
  flying.style.position = "fixed";
  flying.style.left = rect.left + "px";
  flying.style.top = rect.top + "px";
  flying.style.zIndex = 9999;
  flying.style.transition = "all 300ms ease-out";

  document.body.appendChild(flying);

  // Destination = CPU hand area
  const cpuRect = el.cpu.getBoundingClientRect();

  requestAnimationFrame(() => {
    flying.style.left = cpuRect.left + 40 + "px"; // slight offset into fan
    flying.style.top = cpuRect.top + 20 + "px";
...
function animateCpuDiscard(idx, card, onDone) {
    const cpuCards = el.cpu.querySelectorAll(".card");
    const cardElem = cpuCards[idx];
    ...
    requestAnimationFrame(() => {
        flying.style.left = discardRect.left + "px";
        flying.style.top = discardRect.top + "px";
...
/*
function animateCpuDiscard(idx, card) {
  const cpuCards = el.cpu.querySelectorAll(".card");
  const cardElem = cpuCards[idx];
  const rect = cardElem.getBoundingClientRect();
